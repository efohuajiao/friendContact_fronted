<template>
  <van-card
    v-for="(userList, index) in userListInfo"
    :key="index"
  ...
  >
    <template #tags>
      <van-space>
        <van-tag plain type="danger" v-for="(tag, index) in userList.tags" :key="index">
          {{
          tag
          }}
        </van-tag>
  ...
  <!-- 搜索提示 -->
  <van-empty v-if="!userListInfo || userListInfo.length < 1" image="search" description="搜索结果为空" />
  <van-overlay :show="show" @click="show = false">
    <div class="wrapper">
      <div class="block" v-for="(user, index) in userInfo" :key="index">
